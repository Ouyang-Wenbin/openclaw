...
if (-not $PSBoundParameters.ContainsKey("Branch")) {
    if (-not [string]::IsNullOrWhiteSpace($env:OPENCLAW_GIT_BRANCH)) {
        $Branch = $env:OPENCLAW_GIT_BRANCH
    }
}
...
function Install-OpenClawFromGit {
    ...
    if (-not (Test-Path $RepoDir)) {
        git clone -b $BranchName $repoUrl $RepoDir
    } else {
        if (-not $SkipUpdate) {
            git -C $RepoDir fetch origin $BranchName 2>$null
            git -C $RepoDir checkout $BranchName 2>$null
        }
    }

    if (-not $SkipUpdate) {
        if (-not (git -C $RepoDir status --porcelain 2>$null)) {
            git -C $RepoDir pull --rebase origin $BranchName 2>$null
        } else {
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        $finalGitDir = $GitDir
        Install-OpenClawFromGit -RepoDir $GitDir -BranchName $Branch -SkipUpdate:$NoGitUpdate
    } else {
        Install-OpenClaw
    }
